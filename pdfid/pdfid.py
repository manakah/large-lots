...
def PDFiD(file, allNames=False, extraData=False, disarm=False, force=False, outfile=None):
    ...
    XMLAddAttribute(xmlDoc, 'Filename', 'name')
    ...
    try:
        ...
        if disarm:
            for byteHeader in bytesHeader:
                outfile.write(C2BIP3(chr(byteHeader)))
        if oEntropy != None:
            for byteHeader in bytesHeader:
            ...
            if charUpper >= 'A' and charUpper <= 'Z' or charUpper >= '0' and charUpper <= '9':
                ...
                if d1 != None:
                    ...
                    if d2 != None and (chr(d1) >= '0' and chr(d1) <= '9' or chr(d1).upper() >= 'A' and chr(d1).upper() <= 'F') and (chr(d2) >= '0' and chr(d2) <= '9' or chr(d2).upper() >= 'A' and chr(d2).upper() <= 'F'):
                        ...
                        (word, wordExact, hexcode, lastName, insideStream) = UpdateWords(word, wordExact, slash, words, hexcode, allNames, lastName, insideStream, oEntropy, outfile)
                        if disarm:
                            outfile.write(C2BIP3(char))
                else:
                    oBinaryFile.unget(d1)
                    (word, wordExact, hexcode, lastName, insideStream) = UpdateWords(word, wordExact, slash, words, hexcode, allNames, lastName, insideStream, oEntropy, oufile)
                    if disarm:
                        outfile.write(C2BIP3(char))
            else:
